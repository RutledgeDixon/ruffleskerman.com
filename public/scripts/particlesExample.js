 ...
 * @constructor
 */
function PartySystem(canvas, nparticles, size) {
    var igloo = this.igloo = new Igloo(canvas),
        gl = igloo.gl,
        w = canvas.width, h = canvas.height;
    gl.disable(gl.DEPTH_TEST);
    this.worldsize = new Float32Array([w, h]);
    var scale = Math.floor(Math.pow(PartySystem.BASE, 2) / Math.max(w, h) / 3);
 ...
 */
PartySystem.encode = function(value, scale) {
    var b = PartySystem.BASE;
 ...
 */
PartySystem.decode = function(pair, scale) {
    var b = PartySystem.BASE;
    return (((pair[0] / 255) * b +
 ...
 * @returns {PartySystem} this
 */
PartySystem.prototype.initTextures = function() {
    ...
    for (var y = 0; y < th; y++) {
        for (var x = 0; x < tw; x++) {
            var i = y * tw * 4 + x * 4,
                px = PartySystem.encode(Math.random() * w, s[0]),
                py = PartySystem.encode(Math.random() * h, s[0]),
                vx = PartySystem.encode(Math.random() * 1.0 - 0.5, s[1]),
                vy = PartySystem.encode(Math.random() * 2.5, s[1]);
            rgbaP[i + 0] = px[0];
            rgbaP[i + 1] = px[1];
            rgbaP[i + 2] = py[0];
            rgbaP[i + 3] = py[1];
 ...
 * @returns {PartySystem} this
 */
PartySystem.prototype.initBuffers = function() {
    var tw = this.statesize[0], th = this.statesize[1],
        gl = this.igloo.gl,
 ...
 * @returns {PartySystem} this
 */
PartySystem.prototype.setCount = function(n) {
 ...
 */
PartySystem.prototype.getCount = function() {
 ...
 */
PartySystem.prototype.get = function() {
    ...
    for (var y = 0; y < h; y++) {
        for (var x = 0; x < w; x++) {
            var i = y * w * 4 + x * 4,
                px = PartySystem.decode([rgba[i + 0], rgba[i + 1]], s[0]),
                py = PartySystem.decode([rgba[i + 2], rgba[i + 3]], s[0]);
 ...
 * @returns {PartySystem} this
 */
PartySystem.prototype.swap = function() {
 ...
 * @returns {PartySystem} this
 */
PartySystem.prototype.updateObstacles = function() {
    var gl = this.igloo.gl;
 ...
 */
PartySystem.prototype.addObstacle = function(center, radius) {
 ...
 * @returns {PartySystem} this
 */
PartySystem.prototype.step = function() {
    var igloo = this.igloo, gl = igloo.gl;
    gl.disable(gl.BLEND);
 ...
 * @returns {PartySystem} this
 */
PartySystem.prototype.draw = function() {
    var igloo = this.igloo, gl = igloo.gl;
    gl.enable(gl.BLEND);
 ...
 * @returns {PartySystem} this
 */
PartySystem.prototype.frame = function() {
 ...
 * @returns {PartySystem} this
 */
PartySystem.prototype.start = function() {
    if (!this.running) {
        this.running = true;
        this.frame();
    }
 ...
 * @returns {PartySystem} this
 */
PartySystem.prototype.stop = function() {
    this.running = false;
    return this;
};
