...
function initShaders(gl, vshaderId, fshaderId) {
  var program = createProgram(gl, vshaderId, fshaderId);
  if (!program) {
...
function createProgram(gl, vshaderId, fshaderId) {
  ...
  var vertexShader = loadShader(gl, gl.VERTEX_SHADER, vertElem);
  
  var fragElem = document.getElementById( fshaderId );
    if ( !fragElem ) { 
        alert( "Unable to load fragment shader " + fshaderId );
        return null;
    }
  var fragmentShader = loadShader(gl, gl.FRAGMENT_SHADER, fragElem);
